...
This is the autonomous layer for bountykit. It exists to keep long-running hunts disciplined instead of turning the repo into a blind scanner.
...
The mission runner in `core/mission.py` uses these states:
...
`core/lifecycle.py` prefers actual evidence packs over category names alone.
...
That generated JSON can be passed directly into `core/mission.py`.
...
3. `/survey` or direct `python3 core/mission.py ...` execution for the scoped target
