...
Subdomain enum → live host probe → crawl → triage → hunt. Use Disposable CLI/tools or approved workspace tooling. If shallow recon produces no useful signal quickly, rotate instead of deepening by habit.
...
Suggested workspace output structure: `recon/$DOMAIN/{subs,resolved,live,crawled,wayback,nuclei-hits}.txt`
...
```bash
ffuf -u "https://$TARGET/FUZZ" -w /path/to/external/wordlist.txt -mc 200,301,302,403 -fc 404 -ac -o ffuf-results.json
```
