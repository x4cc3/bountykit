---
name: payload-bank
description: Security payloads, bypass tables, WAF evasion, race condition templates. Copy-paste ready. For root causes see exploit-atlas. For validation see verdict-gate.
---

# PAYLOAD BANK

Copy-paste payloads. Organized by bug class.
...
- `wordlists/api-endpoints.txt` — common API paths
- `wordlists/common.txt` — general fuzzing
- `wordlists/params.txt` — parameter names
- `wordlists/raft-medium-dirs.txt` — directory brute
- `wordlists/sensitive-files.txt` — exposed files
