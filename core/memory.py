#!/usr/bin/env python3
"""
memory.py — Cross-session hunt memory system.
...
Usage:
  python3 core/memory.py --save-finding --target target.com --bug-class idor \
      --endpoint /api/v2/users --severity high --notes "horizontal IDOR on user endpoint"
  python3 core/memory.py --save-technique --name "uuid-swap" --bug-class idor \
      --payload '{"user_id":"VICTIM_UUID"}' --notes "Works on UUID-based REST APIs"
  python3 core/memory.py --save-session --target target.com
  python3 core/memory.py --recall --target target.com
  python3 core/memory.py --recall --bug-class ssrf
